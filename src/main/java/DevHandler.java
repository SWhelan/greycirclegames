...
public class DevHandler {
	...
	public static void addDefaultUsers() {
		if(DBHandler.numUsers() == 0){
			...
			User molly = new User(DBHandler.getNextUserID(), "Molly", User.hashPassword(salt, password), salt, "henderson.molly4@gmail.com", new BasicDBList(), false, false, false, false, true, 0);
			DBHandler.createUser(molly);
			salt = User.generateSalt();
			User sarah = new User(DBHandler.getNextUserID(), "Sarah", User.hashPassword(salt, password), salt, "slw96@case.edu", new BasicDBList(), false, false, false, false, true, 0);
