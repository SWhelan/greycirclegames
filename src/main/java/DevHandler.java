...
public class DevHandler {
	...
	public static void addDefaultUsers() {
		if(DBHandler.numUsers() == 0){
			...
			for(int i = 0; i < 5; i++){
				salt = User.generateSalt();
				User test = new User(DBHandler.getNextUserID(), "Test" + Integer.toString(i), User.hashPassword(salt, password), salt, "test" + Integer.toString(i) +"@test.com", new  BasicDBList(), false, false, false, false, 0);
